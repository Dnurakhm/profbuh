...
export default async function JobPage({ params }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      ...
      <Card className="border-none shadow-xl overflow-hidden bg-white mb-8">
        ...
        <CardHeader className="border-b p-8">
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            <div className="space-y-3">
              ...
              <CardTitle className="text-3xl font-bold text-slate-900 leading-tight">{job.title}</CardTitle>
        ...
        <CardContent className="p-8">
          <div className="prose max-w-none mb-10">
            <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center">Описание задачи</h3>
            <p className="text-slate-700 whitespace-pre-wrap leading-relaxed text-lg">{job.description}</p>
          </div>
            ...
            {hasAssignee && (isSelectedAccountant || isJobOwner) ? (
              <div className="space-y-8">
                <div className="bg-green-50 p-6 rounded-xl border border-green-100 flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-green-900 text-lg">Проект активен</h4>
                    <p className="text-green-700">{isJobOwner ? "Вы выбрали исполнителя. Чат открыт." : "Вы исполнитель. Обсудите детали с заказчиком."}</p>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-slate-800">
                    <MessageSquare className="h-5 w-5 text-blue-600" />
                    <h3 className="text-xl font-bold">Чат по проекту</h3>
                  </div>
              ...
              <div className="space-y-6">
                {existingBid ? (
                  <div className="bg-amber-50 p-6 rounded-xl border border-amber-100 flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-amber-900 text-lg">Отклик отправлен</h4>
                      <p className="text-amber-700">Ожидайте решения заказчика.</p>
                    </div>
                  </div>
                ) : (
                  <>
              ...
              <div className="p-8 bg-blue-50 rounded-2xl border border-blue-100 text-center">
                ...
                <h4 className="text-xl font-bold text-blue-900">Ожидание откликов</h4>
                <p className="text-blue-700 mb-6">Выберите исполнителя в личном кабинете.</p>
                <Link href="/dashboard/my-jobs" className="inline-block bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition-all">Управлять откликами</Link>
              </div>
            ) : (
